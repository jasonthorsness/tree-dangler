...
export function DistancePreviewPane({
  width,
  height,
  className,
}: DistancePreviewPaneProps) {
  const {
    state: {
      distancePreview,
      distanceField,
      distanceFieldDimensions,
      distanceFieldMax,
    },
  } = useTreeDanglerState();
    ...
    (ctx: CanvasRenderingContext2D) => {
      ...
      if (distancePreview) {
        ...
        const previewImage = new ImageData(
          previewData,
          distancePreview.width,
          distancePreview.height
        );
        if (
          distancePreview.width === width &&
          distancePreview.height === height
        ) {
