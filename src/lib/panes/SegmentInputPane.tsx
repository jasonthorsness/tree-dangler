...
export function SegmentInputPane({
  width,
  height,
  className,
}: SegmentInputPaneProps) {
  const {
    state: { mask, segments },
    dispatch,
  } = useTreeDanglerState();
    ...
    (ctx: CanvasRenderingContext2D) => {
      ...
      segments.forEach((segment) => {
        ...
        if (segment.text) {
          ...
          const halfWidth =
            ctx.measureText(segment.text).width / 2 + textPadding;
  ...
  const handlePointerMove = useCallback(
    (event: PointerEventData) => {
      ...
      if (mode === "segment") {
        const delta = {
          x: pointer.x - pointerStart.x,
          y: pointer.y - pointerStart.y,
        };
