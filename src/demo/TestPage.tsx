...
const callouts = ['Shared context + reducer', 'Canvas-first panes', 'SSR-safe React components']
  ...
  return (
    <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg shadow-black/50">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-300">CanvasPane</p>
          <p className="text-lg text-white">Shared canvas utility</p>
        </div>
        <div className="text-xs text-slate-300">
          {cursor ? (
            <span>
              Pointer: <span className="font-mono text-emerald-200">{cursor.x}</span>,{' '}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-12">
        <header className="space-y-4">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-400">Tree Dangler</p>
          <h1 className="text-4xl font-semibold text-slate-50 sm:text-5xl">Dev Playground</h1>
          <p className="text-base text-slate-300 sm:text-lg">
            Tailwind is wired up for the standalone demo. Future panes will mount here so we can iterate on interactions
            without affecting the embeddable library package.
          </p>
          <div className="mt-2 rounded-lg border border-emerald-500/30 bg-emerald-950/20 p-3 text-xs text-emerald-200">
            State initialized: {state.mask.points.length} mask points, {state.segments.length} segments
          </div>
        </header>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {callouts.map((label) => (
              <span key={label} className="rounded-full border border-emerald-500/30 px-4 py-1 text-xs text-emerald-200">
                {label}
              </span>
            ))}
          </div>
          <p className="mt-4 text-sm text-slate-300">Swap this layout for live panes as we hit new checkpoints.</p>
        </section>
      </div>
    </div>
  )
